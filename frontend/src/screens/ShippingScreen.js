...
const ShippingScreen = ({history}) => {
    ...
    const cart = useSelector(state => state.cart)                         // fetching address from local storage if present
    const {shippingAddress} =  cart

    const [address,setAddress] = useState(shippingAddress.address)             //setting deafult value from local storage
    ...
    const submitHandler = (e) => {
        e.preventDefault()
        dispatch(saveShippingAddress({address,city,postalCode,country}))            //save address in storage
